...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_bCombinationList(false),m_iCurSel(-1), m_iExpandedItem(-1)
{
...
CControlUI* CListUI::GetItemAt(int nIndex) const
{
    return m_pList->GetItemAt(nIndex);
}
...
bool CListUI::SetItemIndex(CControlUI* pControl, int nIndex)
{
    if( pControl->GetInterface(_T("ListHeader")) != NULL ) return CVerticalLayoutUI::SetItemIndex(pControl, nIndex);
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) return m_pHeader->SetItemIndex(pControl, nIndex);
    ...
    if( !m_pList->SetItemIndex(pControl, nIndex) ) return false;
...
bool CListUI::AddAt(CControlUI* pControl, int nIndex)
{
    ...
    // We also need to recognize header sub-items
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) {
        bool ret = m_pHeader->AddAt(pControl, nIndex);
        m_ListInfo.nColumns = MIN(m_pHeader->GetCount(), UILIST_MAX_COLUMNS);
        return ret;
    }
    if (!m_pList->AddAt(pControl, nIndex)) return false;
    ...
    for(int i = nIndex + 1; i < m_pList->GetCount(); ++i) {
        CControlUI* p = m_pList->GetItemAt(i);
...
bool CListUI::Remove(CControlUI* pControl)
{
    ...
    int nIndex = m_pList->GetItemIndex(pControl);
    if (nIndex == -1) return false;

    if (!m_pList->RemoveAt(nIndex)) return false;

    for (int i = nIndex; i < m_pList->GetCount(); ++i) 
	{
        CControlUI* p = m_pList->GetItemAt(i);
        IListItemUI* pListItem = static_cast<IListItemUI*>(p->GetInterface(_T("ListItem")));
        if (pListItem != NULL) 
...
bool CListUI::RemoveAt(int nIndex)
{
    if (!m_pList->RemoveAt(nIndex)) return false;
...
void CListUI::DoEvent(TEventUI& event)
{
	...
	//组合列表是为了屏蔽列表本身处理滚动条消息
	if (m_bCombinationList)
	{
...
bool CListUI::SelectItem(int nIndex, bool bTakeFocus)
{
...
bool CListUI::SelectItem(CControlUI* pControlUI, bool bTakeFocus)
{
	if (!pControlUI) return false;

	int nIndex = GetItemIndex(pControlUI);
    if (nIndex < 0) return false;
    if (nIndex == m_iCurSel) return true;
	int iOldSel = m_iCurSel;
    ...
    if (!pControlUI->IsVisible()) return false;
    if (!pControlUI->IsEnabled()) return false;

    IListItemUI* pListItem = static_cast<IListItemUI*>(pControlUI->GetInterface(_T("ListItem")));
    if (pListItem == NULL) return false;
    ...
    EnsureVisible(m_iCurSel);
    if (bTakeFocus) pControlUI->SetFocus();
    if (m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
...
bool CListUI::ExpandItem(int nIndex, bool bExpand /*= true*/)
{
...
void CListUI::EnsureVisible(int nIndex)
{
    if( m_iCurSel < 0 ) return;
    RECT rcItem = m_pList->GetItemAt(nIndex)->GetPos();
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("itemsize")) == 0 || _tcscmp(pstrName, _T("columns")) == 0) 
		m_pList->SetAttribute(pstrName, pstrValue);
	else if (_tcscmp(pstrName, _T("combinationlist")) == 0 )
	{
		if (_tcscmp(pstrValue, _T("true")) == 0)
			m_bCombinationList = true;
		else m_bCombinationList = false;
	}
...
CListBodyUI::CListBodyUI(CListUI* pOwner) 
	:m_nColumns(0),m_pOwner(pOwner)
{
	m_szItem.cx = m_szItem.cy = 0;
    ASSERT(m_pOwner);
}

SIZE CListBodyUI::GetItemSize() const
{
	return m_szItem;
}

void CListBodyUI::SetItemSize(SIZE szItem)
{
...
int CListBodyUI::GetColumns() const
{
	return m_nColumns;
}

void CListBodyUI::SetColumns(int nCols)
{
	if (nCols <= 0) return;
	m_nColumns = nCols;
	NeedUpdate();
...
void CListBodyUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
...
void CListBodyUI::SetListPos(RECT rc)
{
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++) 
	{
        ...
        RECT rcCtrl = { iPosX + rcPadding.left, iPosY + rcPadding.top, 
			iPosX + rcPadding.left + sz.cx, iPosY + sz.cy + rcPadding.top + rcPadding.bottom };
...
void CListBodyUI::SetListViewPos(RECT rc)
{
	CControlUI::SetPos(rc);
	rc = m_rcItem;
	...
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) 
	{
		...
		if ((iCount % m_nColumns) == 0)
		{
			...
			for (int it2 = it1; it2 < m_items.GetSize(); it2++) 
			{
				...
				if( nIndex == iCount || (nIndex + 1) % m_nColumns == 0 ) 
				{
		...
		if ((iCount % m_nColumns) == 0) rcTile.left -= m_iChildPadding / 2;
		if (((iCount + 1) % m_nColumns) == 0) rcTile.right += m_iChildPadding / 2;
...
{
	if (m_szItem.cx == 0 || m_szItem.cy == 0) SetListPos(rc);
	else SetListViewPos(rc);
}
...
void CListElementUI::SetIndex(int nIndex)
{
    m_iIndex = nIndex;
}
...
LPCTSTR CListTextElementUI::GetText(int nIndex) const
{
    CUIString* pText = static_cast<CUIString*>(m_aTexts.GetAt(nIndex));
    if( pText ) return pText->GetData();
    return NULL;
}

void CListTextElementUI::SetText(int nIndex, LPCTSTR pstrText)
{
    if( m_pOwner == NULL ) return;
    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    if( nIndex < 0 || nIndex >= pInfo->nColumns ) return;
    while( m_aTexts.GetSize() < pInfo->nColumns ) { m_aTexts.Add(NULL); }

    CUIString* pText = static_cast<CUIString*>(m_aTexts[nIndex]);
    if( (pText == NULL && pstrText == NULL) || (pText && *pText == pstrText) ) return;

	if ( pText ) //by cddjr 2011/10/20
		pText->Assign(pstrText);
	else
		m_aTexts.SetAt(nIndex, new CUIString(pstrText));
    Invalidate();
}
...
CUIString* CListTextElementUI::GetLinkContent(int nIndex)
{
    if( nIndex >= 0 && nIndex < m_nLinks ) return &m_sLinks[nIndex];
    return NULL;
}
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        CUIString strText;//不使用LPCTSTR，否则限制太多 by cddjr 2011/10/20
...
void CListContainerElementUI::SetIndex(int nIndex)
{
    m_iIndex = nIndex;
}

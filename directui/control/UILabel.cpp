...
namespace DirectUI
{
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("memorydata")) == 0)
		{
			SetMemoryData(_tcscmp(pstrValue, _T("true")) == 0);
		}
	...
	CUIString CLabelUI::GetText() const
	{
		try
		{
			if(!m_EnableEffect)
	...
	void CLabelUI::SetMemoryData(bool bMemoryData)
	{
		m_bMemoryData = bMemoryData;
	}

	bool CLabelUI::LoadImage(LPCTSTR bitmap, HBITMAP hBitmap, DWORD mask)
	{
		BITMAP bm;
		::GetObject(hBitmap, sizeof(BITMAP), &bm);

		TImageInfo *pImageInfo = CRenderEngine::LoadImage(bitmap, hBitmap, bm.bmWidth, bm.bmHeight);
		if (!pImageInfo) return false;

		if (m_pManager) 
		{
			m_pManager->AddImage(bitmap, hBitmap, pImageInfo->nX, pImageInfo->nY, pImageInfo);
			SetBkImage(bitmap);
		}		
	...
	bool CLabelUI::LoadImage(LPCTSTR bitmap, LPBYTE pData, DWORD dwSize, DWORD mask)
	{
		TImageInfo *pImageInfo = CRenderEngine::LoadImage(bitmap, pData, dwSize, mask);
		if (!pImageInfo) return false;

		if (m_pManager) 
		{
			SetBkImage(bitmap);
			m_pManager->AddImage(bitmap, pImageInfo->hBitmap, pImageInfo->nX, pImageInfo->nY, pImageInfo);
		}

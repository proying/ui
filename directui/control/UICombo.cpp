...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) 
	{
        ...
        m_pLayout->SetBkColor(0xFF000000);
...
void CComboWnd::EnsureVisible(int nIndex)
{
    if ( m_pOwner->GetCurSel() < 0 ) return;
    m_pLayout->FindSelectable(m_pOwner->GetCurSel(), false);
    RECT rcItem = m_pLayout->GetItemAt(nIndex)->GetPos();
...
bool CComboUI::SelectItem(int nIndex, bool bTakeFocus)
{
...
bool CComboUI::SetItemIndex(CControlUI* pControl, int nIndex)
{
...
bool CComboUI::AddAt(CControlUI* pControl, int nIndex)
{
...
bool CComboUI::RemoveAt(int nIndex)
{
    if (!CContainerUI::RemoveAt(nIndex)) return false;
...
CUIString CComboUI::GetText() const
{
...
CUIString CComboUI::GetDropBoxAttributeList()
{

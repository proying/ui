...
namespace DirectUI
{
	...
	LPBYTE CWindowUI::m_lpResourceZIPBuffer = NULL;

	DUI_BEGIN_MESSAGE_MAP(CWindowUI, CNotifyPump)
	...
	LRESULT CWindowUI::ResponseDefaultKeyEvent(WPARAM wParam)
	{
		if (wParam == VK_RETURN)
		{
			return FALSE;
	...
	UILIB_RESOURCETYPE CWindowUI::GetResourceType() const
	{
	...
	CUIString CWindowUI::GetZIPFileName() const
	{
		return _T("");
	}

	LPCTSTR CWindowUI::GetResourceID() const
	{
		return _T("");
	}
	...
	LRESULT CWindowUI::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		return 0;
	}
	...
	LRESULT CWindowUI::OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		return 0;
	}
	...
	void CWindowUI::OnClick(TNotifyUI& msg)
	{

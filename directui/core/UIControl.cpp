...
CUIString CControlUI::GetName() const
{
    return m_sName;
}
...
CUIString CControlUI::GetText() const
{
    return m_sText;
}
...
void CControlUI::SetFixedXY(SIZE szXY)
{
    m_cXY.cx = szXY.cx;
    m_cXY.cy = szXY.cy;
	m_bFloat? NeedUpdate():NeedParentUpdate();
}
...
CUIString CControlUI::GetToolTip() const
{
    return m_sToolTip;
}
...
const CUIString& CControlUI::GetUserData()
{
    return m_sUserData;
}
...
CUIString CControlUI::GetVirtualWnd() const
{
	CUIString str;
...
CControlUI* CControlUI::ApplyAttributeList(LPCTSTR pstrList)
{
    ...
    while (*pstrList != _T('\0')) 
	{
		...
		{
            LPTSTR pstrTemp = ::CharNext(pstrList);
            while (pstrList < pstrTemp) sValue += *pstrList++;
        }
        ASSERT(*pstrList == _T('\"'));
        if (*pstrList++ != _T('\"')) return this;

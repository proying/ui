...
void DirectUI_API DUI__Trace(LPCTSTR pstrFormat, ...)
{
#ifdef _DEBUG
    CUIString strMsg;
    va_list Args;
...
UINT CWindowWnd::ShowModal()
{
    ...
    if (msg.message == WM_QUIT) 
		::PostQuitMessage((UINT)msg.wParam);
...
void CWindowWnd::SetIcon(UINT nRes)
{
	HICON hIcon = (HICON)::LoadImage(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON,
					(::GetSystemMetrics(SM_CXICON) + 15) & ~15, (::GetSystemMetrics(SM_CYICON) + 15) & ~15,	LR_DEFAULTCOLOR); // 防止高DPI下图标模糊
	ASSERT(hIcon);

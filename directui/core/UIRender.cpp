...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPBYTE pData, DWORD dwSize, DWORD mask)
{
    int x,y,n;
	...
	if (!hBitmap) 
	{
		::MessageBox(0, _T("CreateDIBSection失败"), _T("提示"), MB_OK);
		return NULL;
	}
    ...
    TImageInfo* pImageData = new TImageInfo;
    pImageData->hBitmap = hBitmap;
    pImageData->nX = x;
    pImageData->nY = y;
    pImageData->alphaChannel = bAlphaChannel;

	return pImageData;
}

TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, HBITMAP hBitmap, int nWidth, int nHeight)
{
    bool bAlphaChannel = false;	

    TImageInfo* pImageData = new TImageInfo;
    pImageData->hBitmap = hBitmap;
    pImageData->nX = nWidth;
    pImageData->nY = nHeight;
    pImageData->alphaChannel = bAlphaChannel;
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CUIString& sImageName, \
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 )
	{
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
				...
				{
					if( image_count > 0 )
						DirectUI::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CUIString* sLinks, int& nLinkRects, UINT uStyle)
{
        ...
        if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
            ...
            if( !bLineDraw ) {
                if( bInLink && iLinkIndex < nLinkRects ) {
                    ...
                    CUIString *pStr1 = (CUIString*)(sLinks + iLinkIndex - 1);
                    CUIString *pStr2 = (CUIString*)(sLinks + iLinkIndex);
                    *pStr2 = *pStr1;
                }
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( iLinkIndex < nLinkRects && !bLineDraw ) {
                        CUIString *pStr = (CUIString*)(sLinks + iLinkIndex);
                        pStr->Empty();
            ...
            case _T('i'):  // Italic or Image
                {    
                    ...
                    pstrText++;
					CUIString sImageString = pstrText;
                    int iWidth = 0;
                    int iHeight = 0;
                    while( *pstrText > _T('\0') && *pstrText <= _T(' ') ) pstrText = ::CharNext(pstrText);

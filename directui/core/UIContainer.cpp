...
namespace DirectUI
{
	...
	CControlUI* CContainerUI::GetItemAt(int nIndex) const
	{
	...
	bool CContainerUI::SetItemIndex(CControlUI* pControl, int nIndex)
	{
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			if( static_cast<CControlUI*>(m_items[it]) == pControl ) {
				NeedUpdate();            
	...
	bool CContainerUI::AddAt(CControlUI* pControl, int nIndex)
	{
		if( pControl == NULL) return false;
	...
	bool CContainerUI::RemoveAt(int nIndex)
	{
	...
	int CContainerUI::FindSelectable(int nIndex, bool bForward /*= true*/) const
	{
		// NOTE: This is actually a helper-function for the list/combo/ect controls
		//       that allow them to find the next enabled/available selectable item
		if (GetCount() == 0) return -1;
		nIndex = CLAMP(nIndex, 0, GetCount() - 1);
		if (bForward) 
		{
	...
	void CContainerUI::SetFloatPos(int nIndex)
	{
	...
	DirectUI::CUIString CContainerUI::GetSubControlText( LPCTSTR pstrSubControlName )
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl==NULL)
	...
	const CUIString CContainerUI::GetSubControlUserData(LPCTSTR pstrSubControlName)
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl==NULL)

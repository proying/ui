...
LPCTSTR CMarkupNode::GetAttributeName(int nIndex)
{
    if( m_pOwner == NULL ) return NULL;
    if( m_nAttributes == 0 ) _MapAttributes();
    if( nIndex < 0 || nIndex >= m_nAttributes ) return _T("");
    return m_pOwner->m_pstrXML + m_aAttributes[nIndex].iName;
}

LPCTSTR CMarkupNode::GetAttributeValue(int nIndex)
{
    if( m_pOwner == NULL ) return NULL;
    if( m_nAttributes == 0 ) _MapAttributes();
    if( nIndex < 0 || nIndex >= m_nAttributes ) return _T("");
    return m_pOwner->m_pstrXML + m_aAttributes[nIndex].iValue;
}
...
bool CMarkupNode::GetAttributeValue(int nIndex, LPTSTR pstrValue, SIZE_T cchMax)
{
    if( m_pOwner == NULL ) return false;
    if( m_nAttributes == 0 ) _MapAttributes();
    if( nIndex < 0 || nIndex >= m_nAttributes ) return false;
    _tcsncpy(pstrValue, m_pOwner->m_pstrXML + m_aAttributes[nIndex].iValue, cchMax);
    return true;
}

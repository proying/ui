...
CUIString CPaintManagerUI::m_pStrDefaultFontName;//added by cddjr at 05/18/2012
CUIString CPaintManagerUI::m_pStrResourcePath;
CUIString CPaintManagerUI::m_pStrResourceZip;
...
CUIString CPaintManagerUI::GetInstancePath()
{
...
CUIString CPaintManagerUI::GetCurrentPath()
{
    TCHAR tszModule[MAX_PATH + 1] = { 0 };
...
const CUIString& CPaintManagerUI::GetResourcePath()
{
    return m_pStrResourcePath;
}

const CUIString& CPaintManagerUI::GetResourceZip()
{
...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
    ...
    KillTimer(pControl);
    const CUIString& sName = pControl->GetName();
    if (!sName.IsEmpty()) 
	{
...
{
    LPVOID lpOptionGroup = m_mOptionGroup.Find(pStrGroupName);
    if (lpOptionGroup)
	{
        CStdPtrArray* aOptionGroup = static_cast<CStdPtrArray*>(lpOptionGroup);
...
{
    LPVOID lpOptionGroup = m_mOptionGroup.Find(pStrGroupName);
    if (lpOptionGroup) return static_cast<CStdPtrArray*>(lpOptionGroup);
    return NULL;
}
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
    ...
    if (pControl && pControl->GetManager() == this 
...
bool CPaintManagerUI::SetPostPaintIndex(CControlUI* pControl, int nIndex)
{
    RemovePostPaint(pControl);
    return m_aPostPaintControls.InsertAt(nIndex, pControl);
}
...
bool CPaintManagerUI::UseParentResource(CPaintManagerUI* pPaintManager)
{
    if (!pPaintManager)
	{
        m_pParentResourcePM = NULL;
    ...
    if (pPaintManager == this) return false;
    CPaintManagerUI* pParentPM = pPaintManager->GetParentResource();
    ...
    m_pParentResourcePM = pPaintManager;
    return true;
}
...
HFONT CPaintManagerUI::GetFont(int nIndex)
{
    if (nIndex < 0 || nIndex >= m_aCustomFonts.GetSize()) return GetDefaultFontInfo()->hFont;
    TFontInfo* pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[nIndex]);
...
TFontInfo* CPaintManagerUI::GetFontInfo(int nIndex)
{
    if(nIndex < 0 || nIndex >= m_aCustomFonts.GetSize() ) return GetDefaultFontInfo();
    TFontInfo* pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[nIndex]);
...
const TImageInfo* CPaintManagerUI::GetImage(LPCTSTR lpszBitmap)
{
    TImageInfo* pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(lpszBitmap));
    if (!pImageInfo && m_pParentResourcePM) 
	{
		return m_pParentResourcePM->GetImage(lpszBitmap);
	}
    else 
	{
		return pImageInfo;
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR lpszBitmap, LPCTSTR lpszType, DWORD dwMask)
{
    TImageInfo* pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(lpszBitmap));
    if (!pImageInfo) 
	{
        if (AddImage(lpszBitmap, lpszType, dwMask)) 
		{
            pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(lpszBitmap));
        }
    }
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR lpszBitmap, LPCTSTR lpszType, DWORD dwMask)
{
    TImageInfo* pImageInfo = NULL;
    if (lpszType) 
	{
        if (isdigit(*lpszBitmap))
		{
            LPTSTR pstr = NULL;
            int nIndex = _tcstol(lpszBitmap, &pstr, 10);
            pImageInfo = CRenderEngine::LoadImage(nIndex, lpszType, dwMask);
        }
    }
    else 
	{
        pImageInfo = CRenderEngine::LoadImage(lpszBitmap, (LPCTSTR)NULL, dwMask);
    }

    if (!pImageInfo) return NULL;
    if (lpszType) pImageInfo->sResType = lpszType;

    pImageInfo->dwMask = dwMask;
    if (!m_mImageHash.Insert(lpszBitmap, pImageInfo))
	{
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR lpszBitmap, HBITMAP hBitmap, int nWidth, int nHeight, bool bAlpha)
{
    if (hBitmap == NULL || nWidth <= 0 || nHeight <= 0) return NULL;
	
    TImageInfo* pImageInfo = new TImageInfo;
    pImageInfo->hBitmap = hBitmap;
    pImageInfo->nX = nWidth;
    pImageInfo->nY = nHeight;
    pImageInfo->alphaChannel = bAlpha;
    //data->sResType = _T("");
    pImageInfo->dwMask = 0;
    if (!m_mImageHash.Insert(lpszBitmap, pImageInfo))
	{
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR lpszBitmap, HBITMAP hBitmap, int nWidth, int nHeight, TImageInfo* pImageInfo)
{
    if (hBitmap == NULL || nWidth <= 0 || nHeight <= 0 || !pImageInfo) return NULL;
	    
	TImageInfo* pOldImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(lpszBitmap));
	if (pOldImageInfo)
	{		
		m_mImageHash.Remove(lpszBitmap);
...
bool CPaintManagerUI::RemoveImage(LPCTSTR lpszBitmap)
{
    const TImageInfo* pImageInfo = GetImage(lpszBitmap);
    if (!pImageInfo) return false;
    CRenderEngine::FreeImage(pImageInfo) ;

    return m_mImageHash.Remove(lpszBitmap);
}
...
void CPaintManagerUI::RemoveAllImages()
{
    ...
    for (int i = 0; i< m_mImageHash.GetSize(); i++) 
	{
		...
		{
            pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(key, false));
			if (pImageInfo) 
			{
				CRenderEngine::FreeImage(pImageInfo);
			}
...
void CPaintManagerUI::ReloadAllImages()
{
    bool bRedraw = false;
    TImageInfo* pImageInfo = NULL;
    TImageInfo* pNewImageInfo = NULL;
    for (int i = 0; i< m_mImageHash.GetSize(); i++) 
	{
        if (LPCTSTR bitmap = m_mImageHash.GetAt(i))
		{
            pImageInfo = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
            if (pImageInfo) 
			{
                if (!pImageInfo->sResType.IsEmpty()) 
				{
                    if (isdigit(*bitmap)) 
					{
                        LPTSTR lpszStr = NULL;
                        int nIndex = _tcstol(bitmap, &lpszStr, 10);
                        pNewImageInfo = CRenderEngine::LoadImage(nIndex, pImageInfo->sResType.GetData(), pImageInfo->dwMask);
                    }
                }
				else
				{
                    pNewImageInfo = CRenderEngine::LoadImage(bitmap, (LPCTSTR)NULL, pImageInfo->dwMask);
                }

                if (!pNewImageInfo) continue;
                if (pImageInfo->hBitmap) ::DeleteObject(pImageInfo->hBitmap);

                pImageInfo->hBitmap = pNewImageInfo->hBitmap;
                pImageInfo->nX = pNewImageInfo->nX;
                pImageInfo->nY = pNewImageInfo->nY;
                pImageInfo->alphaChannel = pNewImageInfo->alphaChannel;

                delete pNewImageInfo;
                bRedraw = true;
            }
        }
    }
...
CControlUI* CPaintManagerUI::FindSubControlByClass(CControlUI* pParent, LPCTSTR pstrClass, int nIndex)
{
    if (!pParent) pParent = GetRoot();
    ASSERT(pParent);
    m_aFoundControls.Resize(nIndex + 1);
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI* pThis, LPVOID pData)
{
    ...
    const CUIString& strName = pThis->GetName();
    if (strName.IsEmpty()) return NULL;
    // Add this control to the hash list
    pManager->m_mNameHash.Set(strName, pThis);
...
{
    LPCTSTR lpszName = static_cast<LPCTSTR>(pData);
    const CUIString& strName = pThis->GetName();
    if (strName.IsEmpty()) return NULL;

    return (_tcsicmp(strName, lpszName) == 0) ? pThis : NULL;
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromClass(CControlUI* pThis, LPVOID pData)
{
	...
	{
        int nIndex = -1;
        while (pFoundControls->GetAt(++nIndex) != NULL) ;
        if (nIndex < pFoundControls->GetSize()) pFoundControls->SetAt(nIndex, pThis);
    }
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
	...
	if (uChildRes != 0)
	{
		...
		HWND hTempParent = hWndParent;	//子窗口有些按键响应不到的BUG（比如tab）
		for (int i = 0; i < m_aPreMessages.GetSize(); i++) 
		{

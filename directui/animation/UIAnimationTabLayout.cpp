...
bool CAnimationTabLayoutUI::SelectItem(int nIndex)
{
	...
	for (int it = 0; it < m_items.GetSize(); it++)
	{
		if (it == nIndex)
		{
			...
			if (m_pCurrentControl) m_pCurrentControl->SetVisible(false);
				AnimationSwitch();
		}
	...
	if (m_pManager)
	{
		...
		m_pManager->SendNotify(this, _T("tabselect"), m_nCurSel, iOldSel);
	}
